...
When(/^I fill product inputs on "Edit Product" page with following values:$/, async function (table: DataTable) {
  ...
  if (typeof this.createdProduct.price === 'string'){
    this.createdProduct.price = +(this.createdProduct.price)
  }
  return this.createdProduct
});

When(/^I update product inputs on "Edit Product" page with following values:$/, async function (table: DataTable) {
  const product = table.rowsHash();
  await editProductPage.fillInputs(product);
  this.createdProduct = {...this.createdProduct, ...product}  
  if (typeof this.createdProduct.price === 'string'){
