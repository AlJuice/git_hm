...
class ProductApiService {
  ...
  getCreatedProduct(id?: string) {
    if (!this.createdProducts.length) throw new Error("No stored products");
    if (id) {
  ...
  updateProduct(product: Pick<IProductFromResponse, "_id"> & Partial<IProductFromResponse>) {
    const index = this.findProductIndex(product._id);
    if (index === -1) throw new Error("No product found");
    this.createdProducts[index] = {...this.createdProducts[index], ...product };
  }
