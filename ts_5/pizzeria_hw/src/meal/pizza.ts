...
export class Pizza extends Meal {
  ...
  calculatePrice() {
    const basicToppingsPrice = pizzaReceipts[this.name].prices[this.size] + this.basePrice + 
                               pizzaReceipts[this.name].toppings.reduce((sum, topping) => sum + TOPPINGS[topping], 0);
    const additionalToppingsPrice = this.additionalToppings?.reduce((acc, topping) => acc + TOPPINGS[topping], 0) || 0;
